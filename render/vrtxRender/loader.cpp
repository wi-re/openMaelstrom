...
#include <memory>
#include <utility>
void vrtx::objectLoader::appendMesh(mesh toAdd) {
	meshes.push_back(toAdd);
	dirty = true;
}

void vrtx::objectLoader::appendObject(std::string fileName) {
  LOG_INFO << "Started loading object " << fileName << std::endl;
  auto file = resolveFile(fileName, { get<parameters::config_folder>() });
  auto f_obj = file;
  f_obj.replace_extension(".obj");

  auto [objVertices, objTriangles, edges, minp, maxp] = generation::fileToObj(std::experimental::filesystem::exists(f_obj) ? f_obj : file);
  ...
  //std::cout << "Loading obj file as:" << std::endl;
  //for (auto& t : triangles) {
	 // printf("%d %d %d -> [%.2f %.2f %.2f]@[%.2f %.2f %.2f] - [%.2f %.2f %.2f]@[%.2f %.2f %.2f] - [%.2f %.2f %.2f]@[%.2f %.2f %.2f] -> [%.2f %.2f %.2f]\n",
		//  t.i0, t.i1, t.i2, 
		//  vertices[t.i0].position.x, vertices[t.i0].position.y, vertices[t.i0].position.z, vertices[t.i0].normal.x, vertices[t.i0].normal.y, vertices[t.i0].normal.z,
		//  vertices[t.i1].position.x, vertices[t.i1].position.y, vertices[t.i1].position.z, vertices[t.i1].normal.x, vertices[t.i1].normal.y, vertices[t.i1].normal.z,
		//  vertices[t.i2].position.x, vertices[t.i2].position.y, vertices[t.i2].position.z, vertices[t.i2].normal.x, vertices[t.i2].normal.y, vertices[t.i2].normal.z,
...
void vrtx::objectLoader::appendObject(int32_t idx) {
	auto bobj = get<parameters::boundaryVolumes>()[idx];
	LOG_INFO << "Started loading object " << bobj.fileName.value << std::endl;
	auto file = resolveFile(bobj.fileName.value, { get<parameters::config_folder>() });
	auto f_obj = file;
	f_obj.replace_extension(".obj");
	mesh m;
	if (boundaryObjects.find(f_obj.string()) != boundaryObjects.end()) {
		m = boundaryObjects[f_obj.string()];
	}
	else {
		auto[objVertices, objTriangles, edges, minp, maxp] = generation::fileToObj(std::experimental::filesystem::exists(f_obj) ? f_obj : file);
		auto[texture, min, max, dimension, centerOfMass, inertia] = generation::cudaVolume(f_obj.string());
		auto numVtx = objVertices.size();
		auto numTri = objTriangles.size();
		LOG_INFO << "Vertices:  " << numVtx << std::endl;
		LOG_INFO << "Triangles: " << numTri << std::endl;
		...
		for (auto& v : vertices) {
			v.position -= float4{ centerOfMass.x, centerOfMass.y, centerOfMass.z, 0.f };
		}
		for (int32_t i = 0; i < numTri; ++i) {
			auto[idx1, idx2, idx3] = objTriangles[i];
			triangles.push_back(Triangle(idx1, idx2, idx3, vertices));
		...
		//for (auto& t : triangles) {
		   // printf("%d %d %d -> [%.2f %.2f %.2f]@[%.2f %.2f %.2f] - [%.2f %.2f %.2f]@[%.2f %.2f %.2f] - [%.2f %.2f %.2f]@[%.2f %.2f %.2f] -> [%.2f %.2f %.2f]\n",
			  ...
			  //  t.normal.x, t.normal.y, t.normal.z);
		//}
		LOG_INFO << "Finished loading object " << f_obj << std::endl;
		m = mesh{ vertices, triangles };
		boundaryObjects[f_obj.string()] = m;
	}
	Matrix4x4 Tx, Tx_1;
	cudaMemcpy(&Tx, arrays::volumeBoundaryTransformMatrix::ptr + idx, sizeof(Matrix4x4), cudaMemcpyDeviceToHost);
	cudaMemcpy(&Tx_1, arrays::volumeBoundaryTransformMatrixInverse::ptr + idx, sizeof(Matrix4x4), cudaMemcpyDeviceToHost);

	for (auto& v : m.vertices) {
		v.position.w = 1.f;
		v.position = Tx * v.position;
		v.normal.w = 0.f;
		v.normal = math::normalize3(Tx_1.transpose() * v.normal);
	}
	for (auto &t : m.triangles) {
		t.recalculate(m.vertices);
	}

	meshes.push_back(m);
	dirty = true;
}
vrtx::mesh vrtx::objectLoader::mergeMeshes() {
...
//inline auto hash_value(vrtx::Vertex v);
//inline auto hash_value(vrtx::Triangle t);
// inline auto hash_value(const vrtx::Triangle t);
} // namespace boost
...
// auto hash_value(vrtx::Vertex v) {
//   std::size_t seed = 0x0983274;
//   ::combine(seed, v.normal, v.position);
//   return seed;
// }
// auto hash_value(vrtx::Triangle t) {
...
//   return seed;
// }
// auto hash_value(const vrtx::Triangle t) {
//   std::size_t seed = 0x01892364;
//   ::combine(seed, t.center, t.bottom, t.color, t.normal);
//   ::combine(seed, t.e1, t.e2, t.e3, t.d, t.d2, t.d3);
//   ::combine(seed, t.i0, t.i1, t.i2, t.twoSided);
...
std::size_t vrtx::objectLoader::hash() {
  if (!dirty)
    return stored_hash;
  if (!hashable)
    return SIZE_MAX;
...
vrtx::BVHNode *vrtx::objectLoader::buildBVH(BBoxEntries &work, int depth) {
  if (work.size() < 4) {
...
void vrtx::objectLoader::CreateBVH() {
...
int32_t vrtx::objectLoader::boxCount(BVHNode *root) {
  if (!root->leafNode()) {
...
uint32_t vrtx::objectLoader::triangleCount(BVHNode *root) {
  if (!root->leafNode()) {
    BVHInner *p = dynamic_cast<BVHInner *>(root);
...
void vrtx::objectLoader::depthCount(BVHNode *root, int32_t depth, int32_t &maxDepth) {
  if (maxDepth < depth)
    maxDepth = depth;
  if (!root->leafNode()) {
    BVHInner *p = dynamic_cast<BVHInner *>(root);
...
void vrtx::objectLoader::PopulateCacheFriendlyBVH(const Triangle *pFirstTriangle, BVHNode *root, uint32_t &idxBoxes,
                                            uint32_t &idxTriList) {
...
void vrtx::objectLoader::CreateCFBVH() {
  if (!BVHroot) {
...
void vrtx::objectLoader::buildBVH() {
...
void vrtx::objectLoader::createGPUArrays() {
	if (!active)
		return;
  ...
  //std::cout << "Copying obj file as:" << std::endl;
  //for (auto& t : merged.triangles) {
	 // auto& vertices = merged.vertices;
	 // printf("%d %d %d -> [%.2f %.2f %.2f]@[%.2f %.2f %.2f] - [%.2f %.2f %.2f]@[%.2f %.2f %.2f] - [%.2f %.2f %.2f]@[%.2f %.2f %.2f] -> [%.2f %.2f %.2f]\n",
		//  t.i0, t.i1, t.i2,
		//  vertices[t.i0].position.x, vertices[t.i0].position.y, vertices[t.i0].position.z, vertices[t.i0].normal.x, vertices[t.i0].normal.y, vertices[t.i0].normal.z,
		//  vertices[t.i1].position.x, vertices[t.i1].position.y, vertices[t.i1].position.z, vertices[t.i1].normal.x, vertices[t.i1].normal.y, vertices[t.i1].normal.z,
...
vrtx::objectLoader::~objectLoader() {
...
vrtx::gpuBVH vrtx::objectLoader::getGPUArrays() {
...
void vrtx::objectLoader::tearDownMeshes() {
