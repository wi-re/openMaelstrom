...
#include <render/quadRender/geometry.h>
  ...
  std::tuple<Vertex&, Vertex&, Vertex&> Triangle::getVertices(std::vector<Vertex>& vtx) {
	  return std::tie(vtx[i0], vtx[i1], vtx[i2]);
  }
  std::tuple<float3&, float3&, float3&> Triangle::getVertices(std::vector<float3>& vtx) {
	  return std::tie(vtx[i0], vtx[i1], vtx[i2]);
  }
  std::tuple<Vertex, Vertex, Vertex> Triangle::getVertices(const std::vector<Vertex>& vtx) {
	  return std::make_tuple(vtx[i0], vtx[i1], vtx[i2]);
  }
  std::tuple<float3, float3, float3> Triangle::getVertices(const std::vector<float3>& vtx) {
	  return std::make_tuple(vtx[i0], vtx[i1], vtx[i2]);
  }
  std::pair<float4, float4> Triangle::getAABB() {
