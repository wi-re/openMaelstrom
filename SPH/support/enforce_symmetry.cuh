...
/*
Module used to constrain the neighborhoods of particles in a novel way that works with a single cell iteration by using histograms (Paper pending)
*/
namespace SPH{
	namespace enforceSymmetry{
		struct Memory{
			// basic information
			...
			write_array_u<arrays::MLMResolution> MLMResolution;
			...
			using output_arrays = std::tuple<arrays::position, arrays::MLMResolution>;
			...
			using parameters = std::tuple<parameters::target_neighbors, parameters::support_leeway, parameters::neighborlimit>;
...
constexpr static const bool inlet = false;
		...
		void constrain_support(Memory mem = Memory());
	} // namspace enforceSymmetry
}// namespace SPH
