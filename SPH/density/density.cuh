...
namespace SPH{
	namespace Density{
		struct Memory{
			...
			write_array_u<arrays::matrixTest> matrixTest;
			write_array_u<arrays::lifetime> lifetime;
			write_array_u<arrays::fluidDensity> fluidDensity;
			...
			using input_arrays = std::tuple<arrays::position, arrays::volume, arrays::boundaryPlanes, arrays::particle_type, arrays::rigidDensities>;
			using output_arrays = std::tuple<arrays::density, arrays::matrixTest, arrays::lifetime, arrays::fluidDensity>;
...
constexpr static const bool inlet = false;
		...
		inline bool valid(Memory){
			...
			condition = condition || get<parameters::modules::density>() == "standard";
			return condition;
		}
