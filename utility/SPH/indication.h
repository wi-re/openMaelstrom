...
hostDeviceInline uint64_t mortonEncode_magicbits_32(uint32_t x, uint32_t y, uint32_t z) {
	uint32_t answer = 0;
...
struct position_to_idx3D_helper {
	hostDeviceInline static auto fn(T p, U min, V d) {
		uint32_t i = static_cast<uint32_t>(floorf((math::unit_get<1>(p) - math::unit_get<1>(min)) / d));
		uint32_t j = static_cast<uint32_t>(floorf((math::unit_get<2>(p) - math::unit_get<2>(min)) / d));
		uint32_t k = static_cast<uint32_t>(floorf((math::unit_get<3>(p) - math::unit_get<3>(min)) / d));
		return uint3{ i, j, k };
	}
};
...
struct position_to_idx3D_i_helper {
	hostDeviceInline static auto fn(T p, U min, V d) {
		int32_t i = static_cast<int32_t>(floorf((math::unit_get<1>(p) - math::unit_get<1>(min)) / d));
		int32_t j = static_cast<int32_t>(floorf((math::unit_get<2>(p) - math::unit_get<2>(min)) / d));
		int32_t k = static_cast<int32_t>(floorf((math::unit_get<3>(p) - math::unit_get<3>(min)) / d));
		return int3{ i, j, k };
	}
};
