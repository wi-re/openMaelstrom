...
std::vector<vdb::Vec4f> VolumeToHex(vdb::FloatGrid::Ptr grid, vdb::Vec3d min_vdb, vdb::Vec3d max_vdb, float r) {
  ...
  vdb::Vec4f firstPoint{(float)min_vdb.x() + 0.f * spacing, (float)min_vdb.y() + 0.f * spacing,
                        (float)min_vdb.z() + 0.f * spacing, h};
  ...
  do {
    do { 
      ...
      } while (inserted.x() < max_vdb.x() - 0.f * spacing);
      i = 0;
      ++j;
    } while (inserted.y() < max_vdb.y() - 0.f * spacing);
    j = 0;
    k++;
  } while (inserted.z() < max_vdb.z() - 0.f * spacing);
