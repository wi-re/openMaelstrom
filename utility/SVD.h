...
#define JACOBI_STEPS 14
#define RSQRT_STEPS 6
#define RSQRT1_STEPS 8
...
_mm_setcsr( oldMXCSR__ ); 
	...
	struct Mat3x3 {
		...
		static __host__ __device__ __forceinline__ Mat3x3 fromPtr(float* ptr, int32_t i, int32_t offset) {
			return Mat3x3{
				ptr[i  * 9 + 0 * 1], ptr[i * 9 + 1 * 1], ptr[i * 9 + 2 * 1],
				ptr[i * 9 + 3 * 1], ptr[i * 9 + 4 * 1], ptr[i * 9 + 5 * 1],
				ptr[i * 9 + 6 * 1], ptr[i * 9 + 7 * 1], ptr[i * 9 + 8 * 1]
			};
		}
